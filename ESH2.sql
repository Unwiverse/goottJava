...
create table products(
    ...
    /*
    constraint category_ref: 이 부분은 제약 조건의 이름을 나타냅니다. 
    여기서는 'category_ref'로 정의되어 있습니다. 이 이름은 제약 조건을 식별하는 데 사용됩니다.
    foreign key(category_fk): 'category_fk' 열이 외래 키임을 나타냅니다. 
    이것은 'products' 테이블의 열이 다른 테이블의 키를 참조한다는 것을 의미합니다.
    references category(category_code): 'category_fk' 열이 참조하는 테이블과 열을 나타냅니다. 
    여기서는 'category' 테이블의 'category_code' 열을 참조하도록 설정되어 있습니다. 
    이것은 'products' 테이블의 'category_fk' 열이 'category' 테이블의 'category_code' 열에 
    있는 값만 가질 수 있음을 의미합니다.
    */
);
